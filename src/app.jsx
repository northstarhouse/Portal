...
function Badge({ status }) {
  ...
  return (
    <span style={{ background: style.bg, color: style.color, fontSize: 13, fontWeight: 500, padding: "2px 8px", borderRadius: 20, whiteSpace: "nowrap" }}>
      {status}
    </span>
  );
}
...
function StatCard({ label, value, sub }) {
  return (
    <div style={{ background: "#fff", border: "0.5px solid #e0d8cc", borderRadius: 10, padding: "14px 18px", minHeight: 90, display: 'flex', flexDirection: 'column', justifyContent: 'center', flex: 1, minWidth: 120 }}>
      ...
      <div style={{ fontSize: 13, color: "#777", marginTop: 2, minHeight: 16 }}>{sub || ''}</div>
    </div>
  );
}
    ...
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 14, color: gold, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase" }}>Today — March 20, 2026</span>
          <span style={{ color: '#777', fontSize: 14 }}>—</span>
          <span style={{ fontSize: 15, color: "#888" }}>Here's your organization at a glance.</span>
        </div>
      </div>
      ...
      <div style={{ background: "#fff4e5", border: "0.5px solid #e0c98a", borderRadius: 10, padding: "12px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 500, color: "#8a6200" }}>Quarterly Update Due — March 31, 2026</div>
        </div>
        <div style={{ marginLeft: "auto", fontSize: 13, fontWeight: 500, color: "#c0392b", background: "#fce4e4", padding: "3px 10px", borderRadius: 20 }}>11 days away</div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ background: "#fff", border: "0.5px solid #e0d8cc", borderRadius: 10, padding: "16px 18px" }}>
          <div style={{ fontSize: 14, fontWeight: 500, color: gold, marginBottom: 14, textTransform: "uppercase", letterSpacing: 0.8 }}>Upcoming — NSH Calendar</div>
          {calEvents === null && <div style={{ fontSize: 15, color: "#777" }}>Loading…</div>}
          {calEvents !== null && calEvents.length === 0 && <div style={{ fontSize: 15, color: "#777" }}>No upcoming events in the next 2 weeks.</div>}
          {calEvents !== null && calEvents.map(function(ev, i) {
            ...
            return (
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 500, color: "#2a2a2a" }}>{title}</div>
                  <div style={{ fontSize: 13, color: "#777", marginTop: 2 }}>{dayStr}{timeStr !== 'All day' ? ' · ' + timeStr : ''}</div>
                </div>
                {label && <span style={{ fontSize: 13, background: labelBg, color: dotColor, borderRadius: 20, fontWeight: 500, flexShrink: 0, width: 90, textAlign: 'center', display: 'inline-block', padding: '2px 0' }}>{label}</span>}
              </div>
            );
          })}
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: "0.5px solid #f0ebe2", fontSize: 13, color: "#999" }}>
...
function TeamPicker({ value, onChange }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        {selected.length === 0 && <span style={{ fontSize: 15, color: '#999' }}>Select teams...</span>}
        {selected.map(function(t) {
          return (
            <span key={t} style={{ background: (TEAM_COLORS[t] || { bg: '#f3f3f3' }).bg, color: (TEAM_COLORS[t] || { color: '#555' }).color, fontSize: 13, fontWeight: 500, padding: '2px 8px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
              {t}
              <span
        ...
        <span style={{ marginLeft: 'auto', fontSize: 13, color: '#999', flexShrink: 0 }}>{open ? '▲' : '▼'}</span>
      </div>
      {open && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '0.5px solid #e0d8cc', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.1)', zIndex: 100, marginTop: 4, padding: '6px 0', maxHeight: 220, overflowY: 'auto' }}>
          {TEAM_OPTIONS.map(function(opt) {
            ...
            return (
              ...
              >
                {opt}
                {isOn && <span style={{ color: gold, fontSize: 15, fontWeight: 600 }}>✓</span>}
              </div>
            );
          })}
        </div>
      ...
      <div onClick={function(e) { e.stopPropagation(); }} style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 480, width: '100%', boxShadow: '0 8px 40px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={onSubmit}>
          ...
          <div style={{ display: 'flex', gap: 20, marginBottom: 14 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 15, color: '#444', cursor: 'pointer' }}><input type="checkbox" name="CC" checked={form['CC']} onChange={onChange} /> CC</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 15, color: '#444', cursor: 'pointer' }}><input type="checkbox" name="Nametag" checked={form['Nametag']} onChange={onChange} /> Nametag</label>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            <button type="submit" disabled={saving} style={{ flex: 1, background: gold, color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 15, fontWeight: 500, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Saving...' : 'Save'}</button>
            <button type="button" onClick={onCancel} style={{ flex: 1, padding: 10, background: '#f5f0ea', border: 'none', borderRadius: 8, fontSize: 15, color: '#666', cursor: 'pointer', fontWeight: 500 }}>Cancel</button>
          </div>
        </form>
      </div>
    </div>
  ...
  function InfoRow({ label, value, link }) {
    ...
    return (
      <div style={{ display: 'flex', gap: 0, marginBottom: 10, alignItems: 'flex-start' }}>
        <div style={{ width: 110, fontSize: 14, color: '#777', flexShrink: 0, paddingTop: 1 }}>{label}</div>
        <div style={{ fontSize: 15, color: '#2a2a2a', flex: 1, lineHeight: 1.4 }}>
  ...
  function NoteBlock({ label, value }) {
    ...
    return (
      <div style={{ marginBottom: 10 }}>
        {label && <div style={{ fontSize: 13, color: '#999', fontWeight: 600, letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 5 }}>{label}</div>}
        <div style={{ fontSize: 15, color: '#444', lineHeight: 1.65, background: '#faf8f4', borderRadius: 8, padding: '10px 14px' }}>{value}</div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 0, background: '#f0ebe3', borderRadius: 10, padding: 3 }}>
          ...
          >Active <span style={{ fontSize: 13, color: tab === 'active' ? gold : '#bbb', fontWeight: 500 }}>{active}</span></button>
          <button
          ...
          >Inactive <span style={{ fontSize: 13, color: tab === 'inactive' ? gold : '#bbb', fontWeight: 500 }}>{inactive}</span></button>
        </div>
        <button onClick={function() { setForm(emptyForm); setShowAdd(true); }} style={{ background: gold, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 15, fontWeight: 500, cursor: 'pointer' }}>+ Add Volunteer</button>
      </div>
      ...
      {error && <div style={{ background: '#fce4e4', borderRadius: 8, padding: '10px 14px', marginBottom: 12, fontSize: 15, color: '#c0392b' }}>Error: {error}</div>}

      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: '#777', fontSize: 15 }}>Loading volunteers...</div>
      ) : (
          ...
          {filtered.map(function(v, i) {
            ...
            return (
              <div key={i} onClick={function() { setSelected(v); }}
                ...
                <div style={{ fontSize: 15, fontWeight: 500, color: '#2a2a2a', marginBottom: 3, lineHeight: 1.3 }}>{v['First Name']} {v['Last Name']}</div>
                {v['Team'] && <div style={{ fontSize: 13, color: '#777', marginBottom: 7, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{(v['Team'] || '').split('|')[0].trim()}</div>}
        ...
        <div onClick={function() { setSelected(null); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.38)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div onClick={function(e) { e.stopPropagation(); }} style={{ background: '#fff', borderRadius: 18, maxWidth: 620, width: '100%', boxShadow: '0 12px 48px rgba(0,0,0,0.22)', maxHeight: '92vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #f8f4ec 0%, #f0e8dc 100%)', padding: '28px 28px 20px', borderBottom: '0.5px solid #e8dece', position: 'relative' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {selected['Team'] && <div style={{ fontSize: 14, color: '#9a7f5a', marginBottom: 6, fontWeight: 500 }}>{selected['Team']}</div>}
                  <Badge status={selected['Status'] || 'Active'} />
                  {selected['Overview Notes'] && <div style={{ fontSize: 14, color: '#7a6a55', marginTop: 8, lineHeight: 1.5, fontStyle: 'italic' }}>{selected['Overview Notes']}</div>}
                </div>
              </div>
              <div style={{ position: 'absolute', top: 14, right: 14, display: 'flex', gap: 8 }}>
                <button onClick={function() { openEdit(selected); }} style={{ background: '#fff', border: '0.5px solid #ddd4c4', borderRadius: 7, padding: '5px 12px', fontSize: 14, color: '#7a6a55', cursor: 'pointer', fontWeight: 500 }}>Edit</button>
            ...
            <div style={{ padding: '20px 28px 24px', overflowY: 'auto' }}>
              ...
              <button onClick={function() { setSelected(null); }} style={{ marginTop: 16, width: '100%', padding: '9px', background: 'transparent', border: '0.5px solid #e0d8cc', borderRadius: 8, cursor: 'pointer', fontSize: 15, color: '#999', fontWeight: 500 }}>Close</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ fontSize: 15, color: '#888' }}>{loading ? 'Loading...' : totalDonors + ' donation' + (totalDonors !== 1 ? 's' : '')}</div>
        <button onClick={function() { setForm(emptyDonForm); setShowAdd(true); }} style={{ background: gold, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 15, fontWeight: 500, cursor: 'pointer' }}>+ Add Donation</button>
      </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: '#777', fontSize: 15 }}>Loading donations...</div>
      ) : (
        <div style={{ background: '#fff', border: '0.5px solid #e0d8cc', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 0, padding: '8px 16px', borderBottom: '0.5px solid #e8e0d4', background: '#faf8f4' }}>
            {['Donor', 'Type', 'Amount', 'Date'].map(function(h) {
              return <div key={h} style={{ fontSize: 13, color: '#777', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.8 }}>{h}</div>;
            })}
          </div>
          {donations.filter(function(d) { return filterType === 'All' || d['Donation Type'] === filterType; }).map(function(d, i) {
            ...
            return (
              ...
              >
                <div>
                  <div style={{ fontSize: 15, fontWeight: 500, color: '#2a2a2a' }}>{d['Donor Name']}</div>
                  <div style={{ fontSize: 13, color: '#999', marginTop: 1 }}>{d['Account Type']}</div>
                </div>
                <div>
                  <span style={{ background: tc.bg, color: tc.color, fontSize: 13, fontWeight: 500, padding: '2px 8px', borderRadius: 20, whiteSpace: 'nowrap' }}>{d['Donation Type']}</span>
                </div>
                <div style={{ fontSize: 15, fontWeight: 600, color: '#2a2a2a' }}>{fmtAmount(d['Amount'])}</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 14, color: '#888' }}>{fmtDate(d['Close Date'])}</span>
                  <span style={{ fontSize: 13, fontWeight: 500, padding: '2px 8px', borderRadius: 20, background: acked ? '#e8f5e9' : '#fff8e1', color: acked ? '#2e7d32' : '#8a6200' }}>{acked ? 'Thanked' : 'Pending'}</span>
                </div>
              </div>
            );
          })}
          ...
          <div onClick={function(e) { e.stopPropagation(); }} style={{ background: '#fff', borderRadius: 16, maxWidth: 540, width: '100%', boxShadow: '0 8px 40px rgba(0,0,0,0.18)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ background: 'linear-gradient(135deg, #f8f4ec 0%, #f0e8dc 100%)', padding: '24px 28px 18px', borderBottom: '0.5px solid #e8dece', position: 'relative', borderRadius: '16px 16px 0 0' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                {(function() { var tc = typeColors[selected['Donation Type']] || { bg: '#f3f3f3', color: '#555' }; return <span style={{ background: tc.bg, color: tc.color, fontSize: 13, fontWeight: 500, padding: '2px 8px', borderRadius: 20 }}>{selected['Donation Type']}</span>; })()}
                <span style={{ fontSize: 14, color: '#777' }}>{fmtDate(selected['Close Date'])}</span>
              </div>
            ...
            <div style={{ padding: '20px 28px 24px', overflowY: 'auto', flex: 1 }}>
              <div style={{ marginBottom: 20 }}><span style={{ fontSize: 15, color: '#777', marginRight: 8 }}>Amount</span><span style={{ fontSize: 16, fontWeight: 600, color: gold }}>{fmtAmount(selected['Amount'])}</span></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 32px' }}>
                <div>
                  <span style={sec}>Donor Info</span>
                  {selected['Informal Names'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Goes by</span>{selected['Informal Names']}</div>}
                  {selected['Account Type'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Type</span>{selected['Account Type']}</div>}
                  {selected['Email'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Email</span><a href={'mailto:' + selected['Email']} style={{ color: gold, textDecoration: 'none' }}>{selected['Email']}</a></div>}
                  {selected['Phone Number'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Phone</span>{selected['Phone Number']}</div>}
                  {selected['Address'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Address</span><span style={{ whiteSpace: 'pre-line' }}>{selected['Address']}</span></div>}
                  <span style={sec}>Payment</span>
                  {selected['Payment Type'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Method</span>{selected['Payment Type']}</div>}
                  {selected['Benefits'] && <div style={{ fontSize: 15, marginBottom: 8 }}><span style={{ color: '#777', marginRight: 8 }}>Benefits</span>{selected['Benefits']}</div>}
                  <div style={{ fontSize: 15, marginBottom: 8 }}>
                    <span style={{ color: '#777', marginRight: 8 }}>Acknowledged</span>
                    <span style={{ fontSize: 13, fontWeight: 500, padding: '2px 8px', borderRadius: 20, background: (selected['Acknowledged'] === true || String(selected['Acknowledged']).toUpperCase() === 'TRUE') ? '#e8f5e9' : '#fff8e1', color: (selected['Acknowledged'] === true || String(selected['Acknowledged']).toUpperCase() === 'TRUE') ? '#2e7d32' : '#8a6200' }}>
                      {(selected['Acknowledged'] === true || String(selected['Acknowledged']).toUpperCase() === 'TRUE') ? 'Thanked' : 'Pending'}
                    </span>
                  </div>
                </div>
                <div>
                  {selected['Donation Notes'] && <div style={{ marginBottom: 12 }}><span style={sec}>Donation Notes</span><div style={{ fontSize: 15, background: '#faf8f4', borderRadius: 8, padding: '10px 14px', color: '#444', lineHeight: 1.6 }}>{selected['Donation Notes']}</div></div>}
                  {selected['Donor Notes'] && <div style={{ marginBottom: 12 }}><span style={sec}>Donor Notes</span><div style={{ fontSize: 15, background: '#faf8f4', borderRadius: 8, padding: '10px 14px', color: '#444', lineHeight: 1.6 }}>{selected['Donor Notes']}</div></div>}
                  {selected['Notes'] && <div style={{ marginBottom: 12 }}><span style={sec}>Notes</span><div style={{ fontSize: 15, background: '#faf8f4', borderRadius: 8, padding: '10px 14px', color: '#444', lineHeight: 1.6 }}>{selected['Notes']}</div></div>}
                </div>
              </div>
              <button onClick={function() { setSelected(null); }} style={{ marginTop: 16, width: '100%', padding: '9px', background: 'transparent', border: '0.5px solid #e0d8cc', borderRadius: 8, cursor: 'pointer', fontSize: 15, color: '#999', fontWeight: 500 }}>Close</button>
            </div>
        ...
        <div onClick={function() { setShowAdd(false); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.32)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={function(e) { e.stopPropagation(); }} style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 480, width: '100%', boxShadow: '0 8px 40px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleDonSubmit}>
              ...
              <div style={{ display: 'flex', gap: 20, marginBottom: 14 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 15, color: '#444', cursor: 'pointer' }}><input type="checkbox" name="Acknowledged" checked={form['Acknowledged']} onChange={handleDonFormChange} /> Acknowledged / Thanked</label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 15, color: '#444', cursor: 'pointer' }}><input type="checkbox" name="Salesforce" checked={form['Salesforce']} onChange={handleDonFormChange} /> In Salesforce</label>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                <button type="submit" disabled={saving} style={{ flex: 1, background: gold, color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 15, fontWeight: 500, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Saving...' : 'Save Donation'}</button>
                <button type="button" onClick={function() { setShowAdd(false); }} style={{ flex: 1, padding: 10, background: '#f5f0ea', border: 'none', borderRadius: 8, fontSize: 15, color: '#666', cursor: 'pointer', fontWeight: 500 }}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
  ...
  return (
    <div style={{ border: '0.5px solid #e0d8cc', borderRadius: 8, overflow: 'hidden', marginTop: 4 }}>
      ...
      <div style={{ display: 'flex', gap: 4, padding: '5px 8px', borderBottom: '0.5px solid #f0ebe2', background: '#faf8f4' }}>
        <button type="button" onMouseDown={function(e) { e.preventDefault(); exec('bold'); }} style={{ background: 'none', border: '0.5px solid #e0d8cc', borderRadius: 5, padding: '1px 8px', fontSize: 15, fontWeight: 700, cursor: 'pointer', color: '#444', lineHeight: 1.6 }}>B</button>
        <button type="button" onMouseDown={function(e) { e.preventDefault(); exec('italic'); }} style={{ background: 'none', border: '0.5px solid #e0d8cc', borderRadius: 5, padding: '1px 8px', fontSize: 15, fontStyle: 'italic', cursor: 'pointer', color: '#444', lineHeight: 1.6 }}>I</button>
      </div>
...
function BoardView() {
  ...
  if (loading) return <div style={{ color: '#777', fontSize: 15, padding: 40, textAlign: 'center' }}>Loading…</div>;
  if (loadError) return <div style={{ color: '#c62828', fontSize: 15, padding: 20, background: '#ffebee', borderRadius: 8, border: '0.5px solid #ffcdd2' }}><strong>Supabase error:</strong> {loadError}<br/><br/>Make sure you've run the SQL setup in Supabase and that RLS is disabled on both tables.</div>;

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div style={{ fontSize: 15, color: '#888' }}>{items.length} topic{items.length !== 1 ? 's' : ''}</div>
        <button onClick={function() { setShowAdd(true); }} style={{ background: gold, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 15, fontWeight: 500, cursor: 'pointer' }}>+ Add Topic</button>
      </div>

      {items.length === 0 && <div style={{ color: '#777', fontSize: 15, textAlign: 'center', padding: 40 }}>No voting items yet.</div>}
            ...
            <React.Fragment key={item.id}>
              {showDivider && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '6px 0' }}>
                  ...
                  <span style={{ fontSize: 13, color: '#999', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Closed</span>
                  <div style={{ flex: 1, height: '0.5px', background: '#e0d8cc' }} />
                </div>
              )}
            <div
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginLeft: 16, flexShrink: 0 }}>
                  <span style={{ fontSize: 14, color: '#777' }}>{iv.length}/{BOARD_MEMBERS.length} voted</span>
                  {revealed
                    ? <span style={{ background: '#e8f5e9', color: '#2e7d32', fontSize: 13, fontWeight: 600, padding: '3px 9px', borderRadius: 20 }}>Closed – Decision Made</span>
                    : <span style={{ background: '#fff3e0', color: '#e65100', fontSize: 13, fontWeight: 600, padding: '3px 9px', borderRadius: 20 }}>Open</span>
                  }
                </div>
              </div>
              {revealed && (
                <div style={{ display: 'flex', gap: 14, marginTop: 10 }}>
                  {[['Yes', t.yes, '#2e7d32'], ['No', t.no, '#c62828'], ['Abstain', t.abstain, '#e65100']].map(function(entry) {
                    return <div key={entry[0]} style={{ fontSize: 14 }}><span style={{ color: entry[2], fontWeight: 600 }}>{entry[1]}</span><span style={{ color: '#777' }}> {entry[0]}</span></div>;
                  })}
                </div>
              )}
            </div>
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: cream, zIndex: 1010, overflowY: 'auto', padding: '32px' }}>
          <div style={{ maxWidth: 680, margin: '0 auto' }}>
            <button onClick={function() { setSelected(null); }} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: '#888', fontSize: 15, cursor: 'pointer', marginBottom: 24, padding: 0 }}>← Back to topics</button>
            ...
            {selected.description && (
              <div dangerouslySetInnerHTML={{ __html: selected.description }} style={{ fontSize: 15, color: '#555', lineHeight: 1.6, marginBottom: 16, padding: '12px 14px', background: '#faf8f4', borderRadius: 8, borderLeft: '3px solid ' + gold }} />
            )}

            {selected.attachment_url && (
              <div style={{ marginBottom: 16 }}>
                <a href={selected.attachment_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 15, color: gold, textDecoration: 'none' }}>📎 View Attachment</a>
              </div>
            )}
              ...
              <div>
                    ...
                    return [
                      ...
                      return (
                        <div key={entry.label} style={{ background: entry.bg, border: '1px solid ' + entry.border, borderRadius: 12, padding: '14px 16px', textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 13, color: entry.color, fontWeight: 600, marginTop: 4, opacity: 0.8 }}>{entry.label}</div>
                        </div>
                      );
                    });
                  })()}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 20 }}>
                  {BOARD_MEMBERS.map(function(m) {
                    ...
                    if (!mv) return (
                      <div key={m} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#fafafa', borderRadius: 8, fontSize: 15 }}>
                        <span style={{ fontWeight: 500, color: '#2a2a2a' }}>{m}</span>
                        <span style={{ color: '#999' }}>—</span>
                        <span style={{ color: '#777', fontSize: 14 }}>No vote</span>
                      </div>
                    );
                    var vc = VOTE_COLORS[mv.choice] || { bg: '#f5f5f5', color: '#888' };
                    return (
                      <div key={m} style={{ padding: '8px 12px', background: '#fafafa', borderRadius: 8, fontSize: 15 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontWeight: 500, color: '#2a2a2a' }}>{m}</span>
                          <span style={{ color: '#999' }}>—</span>
                          <span style={{ background: vc.bg, color: vc.color, fontSize: 14, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>{mv.choice}</span>
                          {mv.changed_in_meeting && <span style={{ fontSize: 13, color: '#b45309', background: '#fef3c7', border: '1px solid #fde68a', fontWeight: 600, padding: '2px 8px', borderRadius: 20 }}>Changed in meeting</span>}
                        </div>
                        {mv.note && <div style={{ fontSize: 14, color: '#777', marginTop: 4, fontStyle: 'italic' }}>{mv.note}</div>}
                      </div>
                    );
                  })}
                </div>
                  ...
                  {BOARD_MEMBERS.map(function(m) {
                    ...
                    return (
                      <div key={m} style={{ background: '#fafafa', borderRadius: 10, padding: '10px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: 15, fontWeight: 500, color: mv ? '#aaa' : '#2a2a2a' }}>{m}</span>
                          {mv && <><span style={{ color: '#999' }}>—</span><span style={{ background: vc.bg, color: vc.color, fontSize: 14, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>{mv.choice}</span></>}
                          {mv && mv.changed_in_meeting && <span style={{ fontSize: 13, color: '#b45309', background: '#fef3c7', border: '1px solid #fde68a', fontWeight: 600, padding: '2px 8px', borderRadius: 20 }}>Changed in meeting</span>}
                          {!mv && <span style={{ fontSize: 14, color: '#777' }}>No vote yet</span>}
                        </div>
                        {mv && mv.note && <div style={{ fontSize: 14, color: '#777', marginTop: 4, fontStyle: 'italic' }}>{mv.note}</div>}
                      </div>
                    );
                  })}
                </div>
                <div style={{ marginTop: 20 }}>
                  {!showPostMeeting ? (
                    ...
                    <div style={{ background: '#fafafa', borderRadius: 12, padding: 16 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: '#2a2a2a', marginBottom: 12 }}>Add Post-Meeting Vote</div>
                      <div style={{ marginBottom: 10 }}>
          ...
          <div onClick={function(e) { e.stopPropagation(); }} style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 480, width: '100%', boxShadow: '0 8px 40px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleTopicSubmit}>
              ...
              <div style={bGrp}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6, padding: '8px 12px', border: '0.5px solid #e0d8cc', borderRadius: 8, cursor: 'pointer', background: '#fff', fontSize: 15 }}>
                  <span style={{ background: gold, color: '#fff', borderRadius: 6, padding: '4px 12px', fontSize: 14, fontWeight: 500, flexShrink: 0 }}>{attachUploading ? 'Uploading…' : 'Choose file'}</span>
                ...
                {topicForm.attachment_url && !attachUploading && <div style={{ fontSize: 13, color: '#2e7d32', marginTop: 4 }}>✓ Uploaded</div>}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                <button type="submit" disabled={topicSaving} style={{ flex: 1, background: gold, color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 15, fontWeight: 500, cursor: 'pointer', opacity: topicSaving ? 0.7 : 1 }}>{topicSaving ? 'Saving…' : 'Add Topic'}</button>
                <button type="button" onClick={function() { setShowAdd(false); }} style={{ flex: 1, padding: 10, background: '#f5f0ea', border: 'none', borderRadius: 8, fontSize: 15, color: '#666', cursor: 'pointer', fontWeight: 500 }}>Cancel</button>
              </div>
            </form>
          </div>
...
function StrategyView() {
  ...
  var tabStyle = function(t) {
    return { padding: '6px 14px', fontSize: 14, fontWeight: 500, border: 'none', borderRadius: 20, cursor: 'pointer',
      background: tab === t ? gold : '#f0ebe2', color: tab === t ? '#fff' : '#666' };
  };

  if (loading) return <div style={{ padding: 40, color: '#777', fontSize: 15 }}>Loading…</div>;
                ...
                return (
                  <div key={g.id} style={{ background: '#fff', border: '0.5px solid #e0d8cc', borderRadius: 10, padding: '14px 16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 15, fontWeight: 600, color: '#2a2a2a', marginBottom: 4 }}>{g.title}</div>
                        {g.description && <div style={{ fontSize: 14, color: '#777', lineHeight: 1.5 }}>{g.description}</div>}
                      </div>
                      {tab !== 'three_year_vision' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                          {g.status && <span style={{ background: sc.bg, color: sc.color, fontSize: 13, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>{g.status}</span>}
                    ...
                    {tab !== 'three_year_vision' && !isEdit && (g.lead || g.due_date) && (
                      <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
                        {g.lead && <div style={{ fontSize: 14, color: '#777' }}>Lead: <span style={{ color: '#555' }}>{g.lead}</span></div>}
                        {g.due_date && <div style={{ fontSize: 14, color: '#777' }}>Due: <span style={{ color: '#555' }}>{g.due_date}</span></div>}
                      </div>
                    )}
                    {isEdit && (
                      <div style={{ marginTop: 14, paddingTop: 14, borderTop: '0.5px solid #f0ebe2' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                          <div>
                            <label style={{ fontSize: 13, color: '#888', fontWeight: 500, display: 'block', marginBottom: 4 }}>Status</label>
                          ...
                          <div>
                            <label style={{ fontSize: 13, color: '#888', fontWeight: 500, display: 'block', marginBottom: 4 }}>Lead</label>
                            <input value={editForm.lead} onChange={function(e) { setEditForm(function(f) { return Object.assign({}, f, { lead: e.target.value }); }); }}
                              style={{ width: '100%', padding: '7px 10px', border: '0.5px solid #e0d8cc', borderRadius: 8, fontSize: 15, boxSizing: 'border-box' }} placeholder="Name…" />
                          </div>
                        </div>
                        <div style={{ marginBottom: 10 }}>
                          <label style={{ fontSize: 13, color: '#888', fontWeight: 500, display: 'block', marginBottom: 4 }}>Due Date</label>
...
function QuarterlyView() {
  return (
    <div style={{ color: '#777', fontSize: 15, padding: '40px 0' }}>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: cream, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="nsh-sidebar" style={{ width: 220, background: "#2a2a2e", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh", overflowY: "auto", scrollbarWidth: "none" }}>
        <div style={{ padding: "24px 20px 16px" }}>
          <div style={{ fontSize: 13, color: gold, fontWeight: 600, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 2 }}>North Star House</div>
          <div style={{ fontSize: 14, color: "rgba(255,255,255,0.35)" }}>Command Center</div>
        </div>
        ...
        <nav style={{ flex: 1, padding: "0 8px" }}>
          {modules.map(m => (
            <button key={m.id} onClick={() => setActive(m.id)} style={{
              ...
              fontSize: 15, fontWeight: active === m.id ? 500 : 400,
